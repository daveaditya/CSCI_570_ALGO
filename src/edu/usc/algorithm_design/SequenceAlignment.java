...
//        System.out.println("Final alignment cost :: " + dp[m-1][n-1]);
    ...
    public int[][] backwardSpaceEfficientAlignment(String X, String Y) {
        ...
        for (int j = n - 1; j >= 0; j--) {
            ...
            for (int i = m - 1; i >= 0; i--) {
                B[i][0] = Math.min(
                            Math.min(
                                    SequenceAlignment.MISMATCH_COST[ALPHABETS.indexOf(X.charAt(i - 1))][ALPHABETS.indexOf(Y.charAt(j - 1))] + B[i + 1][1], SequenceAlignment.GAP_PENALTY + B[i + 1][0]
    ...
    public void divideAndConquerAlignment(String X, String Y) {
        ...
        int[][] right = backwardSpaceEfficientAlignment(X, Y.substring(n / 2 + 1, n));
